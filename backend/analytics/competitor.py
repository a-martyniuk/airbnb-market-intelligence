    ...
    def find_competitors(self, target_listing_id: str, db_path: str = "database/airbnb_intelligence.db") -> List[Dict[str, Any]]:
        ...
        for _, row in df.iterrows():
            ...
            competitor_scores.append({
                ...
                "weekend_price": float(row['weekend_price']) if pd.notna(row.get('weekend_price')) and row.get('weekend_price') is not None else round(float(row['price']) * 1.15, 2),
                "weekly_discount": float(row['weekly_discount']) if pd.notna(row.get('weekly_discount')) and row.get('weekly_discount') is not None else 10.0,
                "monthly_discount": float(row['monthly_discount']) if pd.notna(row.get('monthly_discount')) and row.get('monthly_discount') is not None else 20.0,
                "early_bird_discount": float(row['early_bird_discount']) if pd.notna(row.get('early_bird_discount')) and row.get('early_bird_discount') is not None else 5.0,
                "last_minute_discount": float(row['last_minute_discount']) if pd.notna(row.get('last_minute_discount')) and row.get('last_minute_discount') is not None else 15.0,
                "cleaning_fee": float(row['cleaning_fee']) if pd.notna(row.get('cleaning_fee')) and row.get('cleaning_fee') is not None else 15.0,
                "minimum_stay": int(row['minimum_stay']) if pd.notna(row.get('minimum_stay')) and row.get('minimum_stay') is not None else 2,
                "maximum_stay": int(row['maximum_stay']) if pd.notna(row.get('maximum_stay')) and row.get('maximum_stay') is not None else 90,
                "instant_book": bool(row['instant_book']) if pd.notna(row.get('instant_book')) and row.get('instant_book') is not None else True,
                "cancellation_policy": str(row['cancellation_policy']) if pd.notna(row.get('cancellation_policy')) and row.get('cancellation_policy') else "Moderada",
